...
import PersonalInfo from "@components/profile/my_account/profile/personal_information/personal_info";
import AboutMe from "@components/profile/my_account/profile/about_me/about_me";
import BillingAddress from "@components/profile/my_account/profile/billing_address/billing_address";
import ShippingAddress from "@components/profile/my_account/profile/shipping_address/shipping_address";
...
const UserProfile: React.FC = () => {
  ...
  const menuItems = [
    ...
    { id: "billing", icon: "credit-card", label: "Billing", href: "#" },
    { id: "payment", icon: "wallet", label: "Payment", href: "#" },
      ...
      <section className="relative w-full pt-32 md:pt-40 lg:pt-44 pb-16 md:pb-20 bg-gradient-to-r from-red-600 via-red-500 to-red-600 overflow-hidden">
        ...
        <div className="container relative mx-auto px-4 md:px-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8">
            <div className="flex items-center space-x-4 md:space-x-6">
              <div className="relative">
                ...
                <label htmlFor="pro-img" className={`absolute bottom-0 right-0 w-8 h-8 md:w-10 md:h-10 bg-red-500 rounded-full flex items-center justify-center cursor-pointer shadow-lg border-2 border-white transition-colors ${isUploading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-600'}`}>
                  {isUploading ? (
                  ...
                  <input id="pro-img" name="profile-image" type="file" className="hidden" accept="image/jpeg,image/jpg,image/png,image/gif" onChange={handleImageUpload} disabled={isUploading} />
                </label>
              </div>
              ...
              <div className="text-white">
                ...
                <div className="flex items-center space-x-2 text-sm md:text-base">
                  <span className="bg-white/20 px-3 py-1 rounded-full">{userData?.status === 1 ? "Active Member" : "Member"}</span>
                  {userData?.company_name && <span className="bg-white/20 px-3 py-1 rounded-full">{userData.company_name}</span>}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4 md:space-x-8">
              <div className="text-center">
                <div className="text-2xl md:text-3xl font-bold text-white mb-1">{bookingsData?.data?.total || 0}</div>
                <div className="text-red-100 text-sm md:text-base">Total Tours</div>
              </div>
              <div className="h-8 md:h-12 w-px bg-white/30" />
              <div className="text-center">
                <div className="text-2xl md:text-3xl font-bold text-white mb-1">
                  {bookingsData?.data?.data?.filter(b => b.tour_status === 'completed').length || 0}
                </div>
                <div className="text-red-100 text-sm md:text-base">Completed</div>
              </div>
              <div className="h-8 md:h-12 w-px bg-white/30" />
              <div className="text-center">
                <div className="text-2xl md:text-3xl font-bold text-white mb-1">{userData?.rating || 0}</div>
          ...
          <div className="flex flex-wrap gap-3 mt-8 md:mt-12">
            <button onClick={() => setActiveMenu("profile")} className="px-4 md:px-6 py-2 md:py-3 bg-white text-red-600 rounded-lg font-semibold hover:bg-gray-100 transition-colors flex items-center">
      ...
      <div className="container mx-auto px-4 md:px-6 py-8 md:py-12 -mt-8 md:-mt-16 relative z-10">
        <div className="flex flex-col lg:flex-row gap-6 md:gap-8">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 sticky top-24">
              ...
              <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div className="bg-red-500 h-2 rounded-full transition-all duration-500" style={{ width: `${completionPercentage}%` }} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:w-3/4">
            {/* Profile Section - Using separate components */}
            {activeMenu === "profile" && (
              <div className="space-y-6">
                <PersonalInfo customer={userData} />
                <AboutMe description={userData?.description} />
                <div className="grid md:grid-cols-2 gap-6">
                  <BillingAddress customer={userData} />
                  <ShippingAddress customer={userData} />
                </div>
              </div>
            )}
            ...
            {activeMenu === "bookings" && (
              <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
                <div className="flex justify-between items-center mb-6">
                  <h5 className="text-xl font-bold text-gray-800 dark:text-white">My Bookings</h5>
                </div>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm mt-3">
                              ...
                              <div>
                                ...
                                <span className="ml-2 font-semibold text-red-500">${parseFloat(booking.amount.toString()).toFixed(2)}</span>
                              </div>
                              <div>
                ...
                {bookingsData?.data?.last_page > 1 && (
                  <div className="flex justify-center gap-2 mt-6">
                    <button 
                      ...
                      className="px-3 py-1 border rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 dark:hover:bg-slate-700"
                    >
                      Previous
                    </button>
                    <span className="px-3 py-1">
                      Page {bookingsPage} of {bookingsData.data.last_page}
                    </span>
                    <button 
                      onClick={() => setBookingsPage(prev => Math.min(bookingsData.data.last_page, prev + 1))}
                      disabled={bookingsPage === bookingsData.data.last_page}
                      className="px-3 py-1 border rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 dark:hover:bg-slate-700"
                    >
                      Next
                    </button>
