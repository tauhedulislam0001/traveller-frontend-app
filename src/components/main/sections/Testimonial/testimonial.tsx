...
import { CustomerReviewListResponse, CustomerReview } from '@redux/services/customer_review/type';

interface TestimonialProps {
    data?: CustomerReviewListResponse;
}
...
function Testimonial({ data }: TestimonialProps) {
    ...
    // Get random avatar based on name (consistent but varied)
    const getAvatarUrl = (name: string, email: string) => {
        const seed = name || email;
        return `https://ui-avatars.com/api/?background=fb2c36&color=fff&bold=true&size=80&name=${encodeURIComponent(seed)}`;
    };
    ...
    const testimonials = data?.data && data.data.length > 0 
        ? data.data.map((review: CustomerReview) => ({
            id: review.id,
            ...
            avatar: getAvatarUrl(review.full_name, review.email)
          }))
        : [
            { 
                id: 1, 
                ...
                rating: 5, 
                review: "This travel agency made our vacation absolutely perfect! Everything was well-organized and exceeded our expectations.", 
                date: "March 2024",
                review_type: "tour_package",
                avatar: getAvatarUrl("Calvin Carlo", "calvin@example.com")
            },
            { 
                id: 2, 
                name: "Christa Smith", 
                ...
                rating: 5, 
                review: "Exceptional service! They handled all the details so we could just relax and enjoy our trip.", 
                date: "February 2024",
                review_type: "tour_package",
                avatar: getAvatarUrl("Christa Smith", "christa@example.com")
            },
            { 
                id: 3, 
                name: "Jemina Clone", 
                email: "jemina@example.com",
                rating: 4, 
                review: "Professional, reliable, and great value. Will definitely book with them again for our next adventure!", 
                date: "January 2024",
                review_type: "blog",
                avatar: getAvatarUrl("Jemina Clone", "jemina@example.com")
            },
        ];
        ...
        <section className="py-16 md:py-24 bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800">
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-1 pb-12 text-center">
                    <h3 className="mb-4 md:text-4xl text-3xl font-bold text-slate-900 dark:text-white">
                        What Our Travelers Say
                    </h3>
                    <p className="text-slate-600 dark:text-slate-400 max-w-2xl mx-auto text-lg">
                        Hear from our satisfied travelers who experienced unforgettable journeys with Travosy Travel Agency.
                    </p>
                    {data?.data && (
                        <p className="text-sm text-custom-red mt-2">
                            {data.data.length} verified reviews
                        </p>
                    )}
                </div>
                    ...
                    {testimonials.length > 0 ? (
                        <Slider ref={sliderRef2} {...sliderSettings2}>
                            {testimonials.map((testimonial) => {
                                const badge = getReviewTypeBadge(testimonial.review_type);
                                return (
                                    <div key={testimonial.id} className="px-4 py-4 h-full">
                                        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl dark:shadow-gray-900/30 p-8 relative border border-gray-100 dark:border-slate-700 hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 h-full flex flex-col">
                                            ...
                                            <div className="flex-grow">
                                                ...
                                                <div className="h-40 mb-6 overflow-y-auto custom-scrollbar">
                                            ...
                                            <div className="flex items-center pt-4 border-t border-gray-100 dark:border-slate-700">
                                                <img 
                                                    ...
                                                    className="w-12 h-12 rounded-full mr-4 border-2 border-custom-red/30 shadow-md object-cover" 
                                                    alt={testimonial.name}
                                                    loading="lazy"
                                                    onError={(e) => {
                                                        const target = e.target as HTMLImageElement;
                                                        target.onerror = null;
                                                        target.src = `https://ui-avatars.com/api/?background=fb2c36&color=fff&bold=true&size=80&name=${encodeURIComponent(testimonial.name)}`;
                                                    }}
                                                />
                                                <div>
            ...
            {/* Add CSS for slider and custom scrollbar */}
            <style jsx global>{`
                .slick-list {
