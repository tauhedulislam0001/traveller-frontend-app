...
interface HeroBlogDetailsProps {
  blogData?: {
    ...
    reading_time: string;
    duration: string;
  }; // Use your proper type here
}

function HeroBlogDetails({ blogData }: HeroBlogDetailsProps) {

  const blogDetails = {
    title : blogData?.title,
    author: blogData?.created_by_name || "Traveller BD",
    date: blogData?.created_at_formatted || "8th February 2026",
    readTime: blogData?.duration ? `${blogData?.duration} Min Read` : "8 Min Read",
  }
          ...
          <ul className="list-none mt-6">
            <li className="inline-block mx-5">
              <div className="bg-black/30 backdrop-blur-sm px-4 py-2 rounded-lg inline-block">
                ...
                <span className="text-white font-semibold block">{blogDetails.author}</span>
              </div>
            </li>
            <li className="inline-block mx-5">
              <div className="bg-black/30 backdrop-blur-sm px-4 py-2 rounded-lg inline-block">
                <span className="text-white/90 block text-sm">Date :</span>
                <span className="text-white font-semibold block">{blogDetails.date}</span>
              </div>
            </li>
            <li className="inline-block mx-5">
              <div className="bg-black/30 backdrop-blur-sm px-4 py-2 rounded-lg inline-block">
                <span className="text-white/90 block text-sm">Time :</span>
                <span className="text-white font-semibold block">{blogDetails.readTime}</span>
              </div>
            </li>
          </ul>
        </div>
